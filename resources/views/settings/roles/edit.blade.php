        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('user-role.update', ['group' => $role->id]) }}">
                        @method('PUT')
                        @csrf
                        <div class="grid md:grid-cols-3 md:w-1/3 mb-4 items-center">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <x-text-input autocomplete="off" id="name" type="text"
                                              class="mt-1 block w-full md:col-span-2" name="name"
                        ...
                        <div class="grid grid-cols-9 mb-6">
                            ...
                            <div class="col-span-3">
                                ...
                                <div class="flex items-center mb-4">
                                    <input type="checkbox" id="p-{{ $permission->id }}" name="permissions[]"
                                           value="{{ $permission->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded
                                           focus:ring-blue-500 focus:ring-2" @if($role->permissions->whereIn('id',
                                    [$permission->id])->count()) {{
                                    'checked '}} @endif>
                                    <label class="ml-2 text-sm font-medium text-gray-900" for="p-{{ $permission->id }}">
                                        {!! $permission->name !!}
                                    </label>
                                </div>
                                @endforeach
